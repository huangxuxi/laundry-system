...
function initInputForm() {
  ...
  const selItem = document.getElementById('inp-item');
  selItem.innerHTML = '<option value="">-- Pilih atau Kosongkan --</option>' +
    ITEM_CATALOG.map(it => `<option value="${it.nama}">${it.nama}</option>`).join('');

  populateLayanan('');
  document.getElementById('inp-harga-satuan').value = '';
  document.getElementById('inp-total').value = '';
  document.getElementById('qris-nominal').textContent = '—';
}

function populateLayanan(itemNama) {
  const sel = document.getElementById('inp-layanan');
...
function onItemChange() {
  const itemNama = document.getElementById('inp-item').value;
  populateLayanan(itemNama);
}
...
function hitungHarga() {
  const val = document.getElementById('inp-layanan').value;
  const berat = parseFloat(document.getElementById('inp-berat').value) || 0;
  const lay = getLayananData(val);
  if (lay) {
    document.getElementById('inp-harga-satuan').value = rupiah(lay.harga) + '/' + lay.satuan;
    document.getElementById('inp-satuan').value = lay.satuan || 'kg';
    const total = lay.harga * berat;
    document.getElementById('inp-total').value = rupiah(total);
    document.getElementById('qris-nominal').textContent = berat > 0 ? rupiah(total) : '—';
  } else {
    document.getElementById('inp-harga-satuan').value = '';
    document.getElementById('inp-total').value = '';
    document.getElementById('qris-nominal').textContent = '—';
...
function simpanPesanan() {
  ...
  const itemNama = document.getElementById('inp-item').value;
  const layVal = document.getElementById('inp-layanan').value;
  const berat = parseFloat(document.getElementById('inp-berat').value);
  ...
  if (!nama || !hp || !alamat || !layVal || !berat || !tglMasuk || !tglSelesai) {
  ...
  if (metodeBayar === 'tunai' && !waktuBayar) {
    toast('Pilih waktu pembayaran (Lunas/COD)!', 'error');
    return;
  }
  if (berat <= 0) {
    toast('Berat/Qty harus lebih dari 0', 'error');
    return;
  }
  ...
  const order = {
    ...
    item: itemNama || '—',
    layanan: lay.nama, satuan: lay.satuan || 'kg',
    berat, tglMasuk, tglSelesai,
...
function resetForm() {
  ['inp-nama', 'inp-hp', 'inp-alamat', 'inp-berat', 'inp-catatan'].forEach(id => document.getElementById(id).value = '');
  document.getElementById('inp-item').value = '';
  document.getElementById('inp-layanan').value = '';
  document.getElementById('inp-harga-satuan').value = '';
  document.getElementById('inp-total').value = '';
  document.getElementById('qris-nominal').textContent = '—';
  metodeBayar = '';
